...
public class DAG<T> {
    ...
    public int path(Vertex<T> from, Vertex<T> to) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (v.equals(to)) {      // если v это город назначения, то возвращаем из paths готовый ответ для этой вершины
                return paths.get(v);
            }

            for (Vertex<T> adjacent : v.getAdjacent()) {  // перебираем вершины по дугам из v
                if (!added.contains(adjacent)) {
                    queue.offer(adjacent);
                    added.add(adjacent);
                    paths.put(adjacent, paths.get(v) + 1);
                }
            }
        }
        return -1;
